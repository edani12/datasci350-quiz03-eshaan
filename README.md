...
6. Test the chatbot using `ollama run conspiracy`. Verify it handles various topics with the appropriate conspiratorial tone and always includes whispered asides
...
8. From your local terminal, upload the files `streaming_analysis.py` and `streaming_data.txt` to the EC2 instance using `scp -i <key.pem> <file> ubuntu@<public-ip>:~` (note that the `:~` specifies the home directory of your instance)
...
11. From your local terminal, download the `os.txt` and `streaming_analysis.png` files to the `aws` directory on your machine (note the syntax: `scp -i <key.pem> <source> <destination>`)
